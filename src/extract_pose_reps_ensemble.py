...
ENSEMBLE_MODEL_PATHS = [
    os.path.join("models", "rep_segmenter_ensemble_0.keras"),
    os.path.join("models", "rep_segmenter_ensemble_1.keras"),
    os.path.join("models", "rep_segmenter_ensemble_2.keras"),
]
...
FALLBACK_MIN_HIP_RANGE_NORM = 0.06
 
REP_PADDING_SEC = 0.35
...
def assess_signal_quality(hip_y: np.ndarray, knee_deg: np.ndarray) -> float:
    ...
    hip_valid = np.isfinite(hip_y).sum() / len(hip_y) if len(hip_y) > 0 else 0
    knee_valid = np.isfinite(knee_deg).sum() / len(knee_deg) if len(knee_deg) > 0 else 0
...
def get_adaptive_thresholds(signal_quality: float) -> dict:
...
def segments_from_ensemble(probs_mean: np.ndarray,
    ...
    # Create mask: high probability AND high confidence
    ...
    for s, e in segments:
        ...
        # Check average confidence within segment
...
def run_ensemble_segmentation(hip_y_series, knee_deg_series, fps, models):
    
    # Prepare raw series
    hip_y = np.array(hip_y_series, dtype=float)
    knee_deg = np.array(knee_deg_series, dtype=float)
    
    hip_y = pd.Series(hip_y).ffill().bfill().values
    knee_deg = pd.Series(knee_deg).ffill().bfill().values
    
    # Assess signal quality
    signal_quality = assess_signal_quality(hip_y, knee_deg)
    thresholds = get_adaptive_thresholds(signal_quality)
    
    # Build features (same as training: hip_y_norm, knee_deg_norm, hip_vel, knee_vel)
    ...
    # Segment extraction
    knee_series = pd.Series(knee_deg, dtype="float64")
    segs = segments_from_ensemble(
        probs_mean, confidence, fps if fps > 0 else FPS_FALLBACK, thresholds, knee_series
    )
...
for model_path in ENSEMBLE_MODEL_PATHS:
    if not os.path.exists(model_path):
        ...
        print("   Train ensemble with train_rep_segmenter_ensemble.py first.")
        raise SystemExit
...
with mp_pose.Pose(static_image_mode=False,
    ...
    for fname in videos:
        ...
        seg_ensemble = run_ensemble_segmentation(hip_y_series, knee_deg_series, fps, ensemble_models)

        if not seg_ensemble:
            # fallback: rule-based on hip_y
            seg_pred = fallback_segment_from_hip(hip_y_series, knee_deg_series, fps=fps)
        else:
            seg_pred = seg_ensemble
